...
def measureTime(preSorted = False, numTrials = 30):
    ...
    # ax.plot(logN,logMS, label='MergeSort')
    # ax.plot(logN,logQS, label='QuickSort')
    legend = ax.legend(loc='upper left')
    ...
    # Plot runtime - nlogn for mergesort and quicksort.
    NlogN = [(x*numpy.log(x) if x>0 else -6) for x in N]

    plt.figure()
    fig, ax = plt.subplots()
    ax.plot(NlogN,tMergeSort, label='Merge')
    ax.plot(NlogN,tQuickSort, label='Quick')
    legend = ax.legend(loc='upper left')
    plt.title('MergeSort and QuickSort algorithms runtimes versus nlogn')
    plt.xlabel('nlog(n)')
    plt.ylabel('runtime')
    if preSorted:
        plt.savefig('merge_quick_sorted.png', bbox_inches='tight')
    else:
        plt.savefig('merge_quick.png', bbox_inches='tight')
